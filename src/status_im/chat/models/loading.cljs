...
(defn- get-referenced-ids
  ;; TODO: rewrite doc
  "Takes map of `message-id->messages` and returns set of message ids
  which are referenced by the original messages, excluding any `message-id`
  which is already in the original map"
  [message-id->messages]
  (into #{}
        (comp (keep (fn [{:keys [content]}]
                      (let [response-to-id
                            (select-keys content [:response-to :response-to-new])]
